...
//        ca.set(2009, 0, 31);//月份是从0开始的，所以11表示12月
//        Date now = ca.getTime();
//        ca.add(Calendar.MONTH, -1); //月份减1
//        Date lastMonth = ca.getTime(); //结果
//        SimpleDateFormat sf = new SimpleDateFormat("yyyy-MM-dd");
//        System.out.println(sf.format(getStartDayOfMonth(now)));
//        System.out.println(sf.format(getEndDayOfMonth(now)));
...
//    }
    ...
    public static Boolean isBeforeMonth(long time, int num) {
        Calendar calendar = getCalendar();
        int currentWeek = calendar.get(Calendar.WEEK_OF_YEAR) + num;
        calendar.setTime(new Date(time));
        int paramWeek = calendar.get(Calendar.WEEK_OF_YEAR);
